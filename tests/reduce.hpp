...
void run_reduce_single(MPI_Comm comm) {
	...
	auto sum = MPIw::Reduce_recv(comm, std::vector{T(my_rank)}, MPI_SUM);
	assert(sum.size() == 1);
...
void run_reduce_range(MPI_Comm comm) {
	...
	auto got = MPIw::Reduce_recv(comm, to_send, MPI_SUM);
...
void run_allreduce_single(MPI_Comm comm) {
	...
	auto sum = MPIw::AllReduce(comm, std::vector{T(my_rank)}, MPI_SUM);
	assert(sum.size() == 1);
...
void run_allreduce_range(MPI_Comm comm) {
	...
	auto got = MPIw::AllReduce(comm, to_send, MPI_SUM);
	assert(got.size() == std::size_t(comm_size));
...
void run_reduce_single(MPI_Comm comm, int root) {
	int my_rank = MPIw::Comm_rank(comm);
	MPIw::Reduce_send(comm, std::vector{T(my_rank)}, MPI_SUM, root);
}
...
void run_reduce_range(MPI_Comm comm, int root) {
	...
	MPIw::Reduce_send(comm, to_send, MPI_SUM, root);
}
} // namespace slave
